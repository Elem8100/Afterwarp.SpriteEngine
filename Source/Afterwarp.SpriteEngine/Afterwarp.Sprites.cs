...
public class SpriteEngine : Sprite
{
    public SpriteEngine(Sprite Parent) : base(Parent)
    {
        deadList = new();
        VisibleWidth = 1800;
        VisibleHeight = 1600;
    ...
    public void GroupSelect(Rect2 Area, bool Add = false, params Sprite[] Filter)
    {
        if (!Add)
            ClearCurrent();
        Sprite sprite;
...
public class Sprite
{
    public Sprite(Sprite Parent)
    {
        ...
        {
            parent.Add(this);
            if (parent is SpriteEngine)
                Engine = (SpriteEngine)parent;
            else
    ...
    public Sprite(Sprite Parent, Dictionary<string, Texture> TextureLib,
    ...
    public void Init(string ImageName, float X, float Y, int Z = 0, int Width = 100, int Height = 100)
    {
        // this.TextureLib = TextureLib;
        this.imageName = ImageName;
        this.x = X;
    ...
    public SpriteEngine Engine { get; set; }
    ...
    public Rect2 CollideRect;
    private Sprite parent;
    private int tag;
    public BlendingEffect BlendingEffect;
    ...
    public int ImageWidth
    {
        get
        {
            return Game.TextureLib[imageName].Parameters.Width;
        }
    }
    public int ImageHeight
    {
        get => Game.TextureLib[imageName].Parameters.Height;
    }
    ...
    {
        if (!Visible || Game.TextureLib == null)
            return;

        GameCanvas.DrawEx(Game.TextureLib[imageName],
           X + Camera.X - Engine.Camera.X, Y + Camera.Y - Engine.Camera.Y);
    }
...
public class SpriteEx : Sprite
{
    ...
    private void Init()
    {
        ...
        this.FilterLevel = FilterLevel.Linear;
        BlendingEffect = BlendingEffect.Normal;
        for (int i = 0; i < 256; i++)
        {
    ...
    public void Init(string ImageName, float X, float Y, int Z, int PatternWidth, int PatternHeight, int Width = 100, int Height = 100)
    {
    ...
    private byte alpha;
    bool HSLMode;
    public int Hue, Saturation, Lightness;
    ...
    public void TowardToAngle(int Direction256, float Speed, bool DoLookAt, float Delta)
    {
        ...
        if (DoLookAt)
        {
            Angle = (float)Direction256 / (-128f / MathF.PI);
        }
    ...
    public Rect2 GetBoundsRect()
    {
        Rect2 Result = new Rect2();
        Result.Left = (int)X;
        Result.Top = (int)Y;
    ...
    {
        if (!Game.TextureLib.ContainsKey(ImageName)) return;
        switch (SpriteSheetMode)
        {
            case SpriteSheetMode.Single:
                ...
                GameCanvas.DrawEx(Game.TextureLib[ImageName],
                                     ...
                                     DoCenter,
                                     ScaleX, ScaleY,
                                     UInt.ARGB(Alpha, Red, Green, Blue),
                ...
                break;
            // 
            case SpriteSheetMode.Pattern:
                
                GameCanvas.DrawPattern(Game.TextureLib[ImageName],
                                        IntMove ? (int)X + Camera.X + Offset.X - (int)Engine.Camera.X : X + Camera.X + Offset.X - Engine.Camera.X,
                                        IntMove ? (int)Y + Camera.Y + Offset.Y - (int)Engine.Camera.Y : Y + Camera.Y + Offset.Y - Engine.Camera.Y,
                                        PatternIndex,
                ...
                break;
            //
            case SpriteSheetMode.CropRect:
                GameCanvas.DrawCrop(Game.TextureLib[ImageName],
                                     IntMove ? (int)X + Camera.X + Offset.X - (int)Engine.Camera.X : X + Camera.X + Offset.X - Engine.Camera.X,
                                     IntMove ? (int)Y + Camera.Y + Offset.Y - (int)Engine.Camera.Y : Y + Camera.Y + Offset.Y - Engine.Camera.Y,
                                     CropRect,
                                     FlipX, FlipY,
...
public class AnimatedSprite : SpriteEx
{
    ...
    public AnimatedSprite(Sprite Parent, Dictionary<string, Texture> TextureLib,
                string ImageName, float X, float Y, int Z = 0, int Width = 100, int Height = 100) : base(Parent)
    {
        DoAnimate = true;
        PatternWidth = 64;
    ...
    public static void AddFrame(string AnimName, string ImageName, int OriginX, int OriginY, Rect CropArea, int Delay)
    {
    ...
    {  
        ...
        Offset.X = AnimationLib[AnimName][ImageIndex].OriginX;
        Offset.Y = AnimationLib[AnimName][ImageIndex].OriginY;
        if (FlipX)
        {
            Offset.X = (AnimationLib[AnimName][ImageIndex].OriginX * -1) - AnimationLib[AnimName][ImageIndex].CropArea.Width;
        }
        if (FlipY)
        {
            Offset.Y = (AnimationLib[AnimName][ImageIndex].OriginY * -1) + AnimationLib[AnimName][ImageIndex].CropArea.Height;
        }
...
public class ParticleSprite : AnimatedSprite
{
    ...
    public override void DoMove(float Delta)
    {
        ...
        VelocityX = VelocityX + AccelX * UpdateSpeed * Delta;
        VelocityY = VelocityY + AccelY * UpdateSpeed * Delta;
...
public class PlayerSprite : AnimatedSprite
{
    ...
    public virtual void Accelerate(float Delta)
    {
        if (speed != MaxSpeed)
        {
...
public class JumperSprite : PlayerSprite
{
    ...
    public override void Accelerate(float Delta)
    {
        if (Speed != MaxSpeed)
        {
            Speed += Acc;
    ...
    public override void DoMove(float Delta)
    {
        ...
        switch (jumpState)
        {
            ...
            case JumpState.jsFalling:
                Y = Y + VelocityY * Delta;
                VelocityY = VelocityY + JumpSpeed * Delta;
