...
public class Sprite2 : AnimatedSprite
{
    ...
    {
        SpriteSheetMode = SpriteSheetMode.CropRect;
...
public class GameFunc
{
    public static void LoadXml(string FileName, string ImageName, string AnimationName, string StandName, string WalkName, string AttackName)
    {
        ...
        foreach (XmlNode i in node.ChildNodes)
        {
            ...
            {
                AnimatedSprite.AddFrame(AnimationName + "Stand", ImageName, OriginX, OriginY,
                    new Afterwarp.Rect(X, Y, Width, Height), 0);
            }

            if (Name.Substring(0, WalkName.Length) == WalkName)
            {
                AnimatedSprite.AddFrame(AnimationName + "Walk", ImageName, OriginX, OriginY, new Afterwarp.Rect(X, Y, Width, Height), 0);
            }
            if (Name.Substring(0, AttackName.Length) == AttackName)
            {
                AnimatedSprite.AddFrame(AnimationName + "Attack", ImageName, OriginX, OriginY, new Afterwarp.Rect(X, Y, Width, Height), 0);
            }
        }
    }
    public static void CeateGame()
    {
        ...
        for (int i = 0; i <= 2; i++)
        {
            var Sprite2 = new Sprite2(Game.SpriteEngine);
            Sprite2.Init("", 100, 140 + 140 * i);
            Sprite2.ID = i;
