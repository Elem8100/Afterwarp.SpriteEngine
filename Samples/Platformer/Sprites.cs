...
public class Player : JumperSprite
{
    ...
    public override void DoMove(float Delta)
    {
        ...
        Keyboard.GetState();
        if (Keyboard.KeyDown(Keys.Right) && State != State.Die)
        {
        ...
        if (Keyboard.KeyDown(Keys.Left) && State != State.Die)
        {
        ...
        if (Keyboard.KeyUp(Keys.Right) && State != State.Die)
        {
            State = State.StandRight;
            if (JumpState == JumpState.jsNone)
                SetAnim("Idle.png", 0, 12, 0.25f, true, false, true);
        }
        if (Keyboard.KeyUp(Keys.Left) && State != State.Die)
        {
        ...
        {
            if ((Keyboard.KeyDown(Keys.LeftControl)) || (Keyboard.KeyDown(Keys.Space)))
            {
                DoJump = true;
                AnimPos = 0;
                PatternIndex = 0;
...
public class GreenApple : JumperSprite
{
    ...
    public static void Create(float PosX, float PosY)
    {
        ...
        {
            var GreenApple = new GreenApple(Game.SpriteEngine);
            GreenApple.Init("Fruit" + (Random.Next(2, 4)).ToString() + ".png", PosX, PosY, 5);
...
public class Background : BackgroundSprite
{
    ...
    public override void DoMove(float Delta)
    {
        ...
        switch (Layer)
        {
            case 1: X = Game.SpriteEngine.Camera.X * 0.5f; break;
            case 2: X = Game.SpriteEngine.Camera.X * 0.3f; break;
            case 3: X = Game.SpriteEngine.Camera.X * 0.1f; break;
        }
    }
}
...
public class GameFunc
{
    public static void SprayBox(float PosX, float PosY)
    {
        ...
        for (int i = 0; i <= 6; i++)
        {
            ...
            Particle.Init("Star.png", PosX + Random.Next(0, 21), PosY + Random.Next(0, 21), 20, 13, 14);
            Particle.SpriteSheetMode = SpriteSheetMode.Single;
            Particle.LifeTime = 150;
            Particle.Decay = 1f;
            Particle.ScaleX = 1.2f;
            Particle.ScaleY = 1.2f;
            Particle.UpdateSpeed = 0.7f;
            Particle.VelocityX = -3 + Rnd() * 7;
            Particle.VelocityY = -Rnd() * 8f;
            Particle.AccelX = 0f;
            Particle.AccelY = 0.15f + Rnd() / 2;
        }
    }
    ...
    public static void SprayFruit(float PosX, float PosY)
    {
        for (int i = 0; i <= 15; i++)
        {
            var Spray = new Spray(Game.SpriteEngine);
            Spray.Init("flare.png", PosX, PosY, 20);
            Spray.ScaleX = 0.1f;
            Spray.ScaleY = 0.1f;
            ...
            Spray.BlendingEffect = BlendingEffect.Add;
        }
    }
    ...
    public static void CreateMap()
    {
        for (int i = 3; i >= 1; i--)
        {
            var Background = new Background(Game.BackgroundEngine);
            Background.Init("back" + i.ToString() + ".png", 0, 0, -100, 0, 0, 1024, 240);
            Background.Layer = i;
            switch (i)
            {
        ...
        Player.Init("Idle.png", 800, 400, 100);
        ...
        for (int i = Length - 2; i > 0; i--)
        {
            ...
            {
                var Tile = new Tile(Game.SpriteEngine);
                Tile.Init(ImageName, X - 540, Y - 20, -10);
            ...
            else
            {
                ...
                Enemy.Init(ImageName, X - 540, Y - 10, 150);
                switch (ImageName)
                {
